        ...
        private static void Input()
{
    ...
    // Only change direction if the pressed key is not the opposite of the current direction
    if (key == ConsoleKey.UpArrow && _direction != Direction.Down) { _direction = Direction.Up; }
    else if (key == ConsoleKey.DownArrow && _direction != Direction.Up) { _direction = Direction.Down; }
    else if (key == ConsoleKey.LeftArrow && _direction != Direction.Right) { _direction = Direction.Left; }
